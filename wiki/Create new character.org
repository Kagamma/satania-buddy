...
- 2D skeletal is the recommended way to create characters for ~satania-buddy~. This is how I created ~satania~, although the downside is that ~Spine~ is not free. You can also try ~Dragon Bones~, which is a free alternative to Spine and can export data in Spine JSON format. Since CGE has it's own Spine runtime implementation, there are limitations compare to official Spine runtime, see [[https://castle-engine.io/spine]] (_Update_: ~satania-buddy~ now come with it's own official spine runtime 4.1 integration, so if you use Spine 4.0 or above to create models, then CGE's limitations will not apply. ~Dragon Bones~ still works, but you need to use CGE's ~view3dscene~ to convert your model from .json to .x3d/x3dv in order to use with ~satania-buddy~)
