...
- Create a new sprite with name
*sprite_other_render(name: string, source: string, format: string)*
- Render sprite ~name~ with content from ~source~. If ~name~ doesn't exist yet, create one.
...
- Example display a 3D yellow cube with a point light source, rotating at character's feet, using x3dv:
