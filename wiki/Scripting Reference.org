...
Note: While we allow the declaration of nested functions, they behave exactly like completely separate functions. Since we do not support closures, they cannot access any local variables from the parent function.
...
- There are limitations in how ~self~ is passed to the calling function. Only a map returned from the nearest function call can be passed.
