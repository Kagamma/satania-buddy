...
- Converts ~count~ floats from buffer to map.
*buffer_to_array_f64(b: buffer, count: number): map*
- Converts ~count~ doubles from buffer to map.
*array_to_buffer_f32(b: buffer): map*
- Converts a map to a buffer. The buffer's type of data is float.
*array_to_buffer_f64(b: buffer): map*
- Converts a map to a buffer. The buffer's type of data is double.
...
- Loads an .evil scheme file in ~data/scripts/current_character~. This will also stop the current script.
...
- Retursn array of chat message. Each item has the following format: { name: string, message: string, timestamp: string }
