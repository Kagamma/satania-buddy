...
- Get HTML result from url_fetch / url_upload. Result map contains status and data. Depends on ~Content-Type~, data can be either string or buffer: /[ "status": number, data: string | buffer ]/
