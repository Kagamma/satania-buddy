...
** Common functions
*typeof(v: any): string*
- Return type of variable (number / boolean / string / map / array / buffer / null).
...
- Run a process by name. Return the same parameter passed to the function.
...
- Return process's output pipeline. This function should be called after process_is_running() returns false.
