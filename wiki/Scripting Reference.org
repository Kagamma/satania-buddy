...
With assertions on, ~assert~ tests if expr is false, and if so, aborts the script with an EAssertionFailed exception. If expr is true, sscript execution continues normally.
...
- Return process's actuao id, status and stdout / stderr pipes: /[ "process_id": number, "running": boolean, "output": null / string ]/
